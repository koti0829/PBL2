...
**手順の流れ**: 道具を準備 → コピーを取得 → 最新版に更新 → Tuesday JSで編集 → テストプレイ → GitHubへ送信

内容: 1.道具を準備する　2.リポジトリを取得する　3.最新版を取得する　4.Tuesday JSで編集する　5.テストプレイの仕方　6.ファイルの構成　7.素材ファイルの保存場所　8.GitHubへ送信する　9.困ったときは

> **すぐ遊びたい人へ:** `professor-galgame` フォルダの中の **`index.html` をダブルクリック**すれば、そのまま全編プレイできる。インストールもエディタもネット接続もいらない。詳しくは「5. テストプレイの仕方」を参照。
...
### A. 全編を通しでプレイする（いちばん簡単）

1. フェーズ2で取得した `PBL2 → professor-galgame` フォルダを開く。
2. その中の **`index.html` をダブルクリック**する。ブラウザが開き、タイトル画面から最後までそのまま遊べる。

インストール作業もエディタもネット接続も不要。他の人に遊んでもらうときも、この方法がいちばん確実。

> **注意①:** `index.html` を**単体で他の場所へ移動・コピーしないこと**。背景や立ち絵を同じフォルダ内の `data` から読み込んでいるため、`index.html` だけ切り離すと画像が表示されなくなる。渡すときは **`professor-galgame` フォルダごとZIPにまとめて**渡す。
>
> **注意②:** `index.html` は、全4章を1つに合体させた版から作られている。**自分がさっき編集した内容は、合体作業が行われるまで反映されない。** 自分の編集をすぐ確認したいときは、下のBの方法を使う。

### B. 自分の担当章だけ確認する

編集中の内容をその場で確認したいときは、Tuesday JSエディタの機能を使う。

1. フェーズ4の手順で、自分の担当ファイル（`ch2.json` など）をTuesday JSで開く。
2. 画面右側のメニューから **`テストプレイ`** をクリックする。
3. その場でゲームが動くので、分岐や演出を確認する。

> **メモ:** この方法で確認できるのは自分の担当章だけ。章をまたぐ動き（例：2年生編から3年生編へ進む部分）は、Aの方法で全体版を確認する。

### 音が出ない・画像が出ないときは

- **音が出ない**: ブラウザのタブがミュートになっていないか、パソコンの音量が0になっていないかを確認する。
- **画像が出ない**: `index.html` を単体で移動していないか確認する（注意①）。`professor-galgame` フォルダごと開き直せば直る。

## 6. ファイルの構成

同じフォルダにJSONファイルが並んでいるが、**編集してよいものと、自動で作られるものがある**。

| ファイル | 中身 | 編集 |
|---|---|---|
| `ch1.json` 〜 `ch4.json` | 各学年編のシナリオ。**担当者が編集するのはこれだけ** | する |
| `game_complete.json` | 上の4つを1つに合体させた全体版 | **しない**（自動生成） |
| `index.html` | 合体版から作った、そのまま遊べるHTML | **しない**（自動生成） |
| `game_sinario.txt` / `問題.txt` | 台本・小テスト問題の元原稿 | 資料 |

> **メモ:** `game_complete.json` と `index.html` は、章の合体作業をしたときにまとめて作り直される。ここを直接編集しても、次の合体で上書きされてしまうので触らないこと。

> **補足:** この他に、画像も含めて全部を1ファイルに詰め込んだ `game_standalone.html`（約39MB）も作れる。ただし**サイズが大きいためGitHubには置いていない**ので、リポジトリを取得しても入っていない。「フォルダごとではなく、ファイル1個だけ提出したい」という場合に用意する。

### 全4章を合体・公開用HTMLに作り直す

全員の章がGitHubに揃ったら、`professor-galgame`フォルダで次を順番に実行する（Python 3が必要。追加インストールは不要）。
...
`game_complete.json` ・ `index.html` ・ `game_standalone.html` がまとめて最新の内容に作り直される。あとはフェーズ8の手順で送信すれば、GitHub Pages（`https://koti0829.github.io/PBL2/`）の公開版にも反映される。詳しい仕組みは [WORKFLOW.md](./WORKFLOW.md) を参照。

## 7. 素材ファイルの保存場所

背景画像・立ち絵・効果音は、専用のフォルダにファイルを置くだけでよい（JSON側の編集は不要）。

```
professor-galgame/
└── data/
    ├── bg/       ← 背景画像（校舎・教室などの写真）
    ├── chara/    ← 立ち絵（教授3人の通常・照れ顔など）
    ├── sound/    ← 効果音・BGM
    ├── title/    ← タイトル画面のイラスト
    └── ex/       ← 小道具の画像（レーザーポインタなど）
```
...
> **メモ:** これで完了。以後は**フェーズ3（最新版を取得）→ フェーズ4（編集）→ フェーズ5（テストプレイ）→ フェーズ8（送信）**の繰り返しになる。
...
で最新版に戻し、退避しておいた内容をもう一度反映して保存し、フェーズ8の手順で送信し直す。
